...
export default async function chinookRegionalHospital() {

const baseUrl = process.env.BASE_URL;
 
  const hospital = Hospitals.filter((hospital) => hospital.name === 'Chinook Regional Hospital')[0];


const responseHr = await fetch(`${baseUrl}/api/hourlyGraph`, {next: { revalidate: 120 }});
const fetchedDataHr = await responseHr.json();
const filteredDataHr = fetchedDataHr.updatedData.filter((item) => item.slug === 'chinookRegionalHospital');
...
const filteredDataDay = fetchedDataDay.averageWaitTimes.filter((item) => item.slug === 'chinookRegionalHospital');
...
const filteredDataWeek = fetchedDataWeek.averageWaitTimes.filter((item) => item.slug === 'chinookRegionalHospital');
