...
export async function GET() {
  try {
    ...
    const pastDate = new Date(currentDate.getTime() - 12 * 60 * 60 * 1000); // Last 12 hours in milliseconds

    const thirtyMinutesInMillis = 30 * 60 * 1000;
    const twelweHoursInMillis = 12 * 60 * 60 * 1000;
    const response = [];
    const promises = [];

    for (let i = pastDate.getTime(); i < currentDate.getTime(); i += twelweHoursInMillis) {
      const startInterval = new Date(i);
      const endInterval = new Date(i + twelweHoursInMillis);
    ...
    for (let i = 0; i < intervalData.length; i++) {
      ...
      const startInterval = new Date(pastDate.getTime() + i * twelweHoursInMillis);
