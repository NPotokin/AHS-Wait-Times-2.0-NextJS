...
export default function AverageMonth(){

    const fetcher = (...args) => fetch(...args).then(res => res.json())
    const { data, error, isLoading } = useSWR(`/api/monthlyAvg`, fetcher, { refreshInterval: 60000 }, { refreshWhenHidden: true } )
    if (error) return <div>ERROR</div>
    if (isLoading) return <LoadingAverage/>
    const filteredDataMonth = data.avgMonth;
    ...
    return(
        ...
        Monthly Average for Alberta
        <p className='font-bold text-3xl py-2'>{filteredDataMonth} min</p>
        </div>
    )
